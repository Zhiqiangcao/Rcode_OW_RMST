###note: censoring distribution is generated based on log-logistic distribution, 
###while using Cox regression to estimate that
  ...
  for(L in L_set){
    ...
    if (gamma == 1){
      if (L == 2) {
        ...
        true_trun_val1 <- true_trun_val2 <- true_trun_val3 <- c(1.498194346,1.776465598,-0.278271251)
      } else if (L == 5) {
        ...
        true_trun_val1 <- true_trun_val2 <- true_trun_val3 <- c(2.68834376,3.803739793,-1.115396033)
      } else if (L == 10) {
        ...
        true_trun_val1 <- true_trun_val2 <- true_trun_val3 <- c(3.580969651,6.127871625,-2.546901973)
      } 
    } else if (gamma == 3) {
      if (L == 2) {
        true_iptw_val <- c(1.498194346,1.776465598,-0.278271251)
    ...
    a0 <- a0_cand[gamma]
    sigma_trtcon = 1;  #scale parameter in log-logistic distribution
    ...
    for(i in 1:nsim){
      ...
      #for censoring time
      lambdac_trt <- cbind(1,x) %*% c(2.0,-0.3,0.5,0.5,0.2,-0.4,-0.5)
      lambdac_crl <- cbind(1,x) %*% c(2.0,-0.3,0.5,0.5,0.2,-0.4,-0.5)
      uc_trt <- rlogis(N); uc_crl <- rlogis(N) #log-logistic distribution
      c_trt <- exp(lambdac_trt+sigma_trtcon*uc_trt) #C^1 for treatment
      c_crl <- exp(lambdac_crl+sigma_trtcon*uc_crl) #C^0 for control
      #T = z*T^1+(1-z)*T^0
      t = z*t_trt + (1-z)*t_crl; 
      #C = z*C^1+(1-z)*C^0
      c = z*c_trt + (1-z)*c_crl; 
    ...
    rownames(res) = c("ow","iptw","strim1","strim2","strim3","astrim1","astrim2","astrim3",
                      "trun1","trun2","trun3")
    write.csv(res, paste0('sim_L_', L, '_G', gamma,'log_logistic_g_Cox_e','.csv'))
  }
}
