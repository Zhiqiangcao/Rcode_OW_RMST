...
RMST.Effect.BW = function(Data,L,Treatment,SurvTime,Status,ps.formula,
  ...
  if(px == 1){  #in this case, no covariate in censoring model, since x from model.matrix is a n*1 matrix 
    x = x       #(intercept) with all values=1, then estimate of x (intercept) will be NA in censored model 
  }else{
    x = matrix(x[,-1],ncol=px-1) #delete the first column of x, since that is intercept and no intercept in Cox model
  }
  px = dim(x)[2]
