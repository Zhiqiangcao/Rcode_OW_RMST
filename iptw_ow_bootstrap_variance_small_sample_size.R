###Additional simulation results: evaluate finite sample properties under small sample size  
###using bootstrap
...
N <- 250     #small sample size, 100 or 250 
bnsim <- 200  #bootstrap times
...
for(gamma in gamma_set){
  for(L in L_set){
    ...
    for(i in 1:nsim){
      ...
      ###the following is nonparametric bootstrap (bnsim times)
      ow_b = iptw_b = matrix(0, ncol=3, nrow=bnsim)
      for(k in 1:bnsim){
        index = sample(1:N,N,replace = TRUE, prob = rep(1/N,N))
        Datab = Data[index,]
        #OW
        Delta.OW_b = RMST.Effect.BW(Datab,L,Treatment,SurvTime,Status,ps.formula,
                                  censor.formula,Method="OW")
        ow_b[k,] = Delta.OW_b$Estimate
        #IPTW
        Delta.IPTW_b = RMST.Effect.BW(Datab,L,Treatment,SurvTime,Status,ps.formula,
                                    censor.formula,Method="IPTW")
        iptw_b[k,] = Delta.IPTW_b$Estimate
      }
      #bootstrap ow_se
      ow_se[i,] = apply(ow_b,2,sd) 
      iptw_se[i,] = apply(iptw_b,2,sd) 
    ...
    write.csv(res, paste0('sim_bootstrap_var_L_', L, '_G', gamma,'_n', N,'.csv'))
  }
}
