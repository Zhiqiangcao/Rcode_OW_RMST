...
visualizedata <- function(gampar,choice){
  ...
  # histograms
  if(choice==1){
    hist(e[z==1],breaks=50,col="gray77",border="gray77",main=paste("(a) strong overlap"),
         xlim=c(0,1),ylim=c(0,3),xlab="Propensity score",freq=F,
         cex.lab = 0.8, cex.axis = 0.8 ,cex.main = 0.8, cex = 0.8)
    hist(e[z==0],breaks=50,col="NA",add=TRUE,freq=F)
  }else if(choice==2){
    hist(e[z==1],breaks=50,col="gray77",border="gray77",main=paste("(b) moderate overlap"),
         xlim=c(0,1),ylim=c(0,3),xlab="Propensity score",freq=F,
         cex.lab = 0.8, cex.axis = 0.8 ,cex.main = 0.8, cex = 0.8)
    hist(e[z==0],breaks=50,col="NA",add=TRUE,freq=F)
  }else if(choice==3){
    hist(e[z==1],breaks=50,col="gray77",border="gray77",main=paste("(c) weak overlap"),
         xlim=c(0,1),ylim=c(0,7),xlab="Propensity score",freq=F,
         cex.lab = 0.8, cex.axis = 0.8 ,cex.main = 0.8, cex = 0.8)
    hist(e[z==0],breaks=50,col="NA",add=TRUE,freq=F)
  }
}
