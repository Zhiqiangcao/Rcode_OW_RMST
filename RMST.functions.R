...
cal.rmst = function(Timevec,L,sf.est){
...
CensorScore.est = function(Coxmodel,x,z,delta,n){
...
OW.RMST = function(L,w,x,z,time,delta,ps.model,cen.trt.model,cen.con.model){
  ...
  # censoring function
  n = length(z)
  Kc.trt.est = CensorScore.est(cen.trt.model,x,z,delta,n)   #for treatment
  Kc.con.est = CensorScore.est(cen.con.model,x,1-z,delta,n) #for control
  ...
  auc1 = cal.rmst(time,L,surv.trt.est)
  ...
  auc0 = cal.rmst(time,L,surv.con.est)
  # for control group
...
IPTW.RMST = function(L,w,x,z,time,delta,ps.model,cen.trt.model,cen.con.model){
  # w is covariates vector for PS model, including the intercept term
  # x is covariates vector for censoring model
  # propensity score
  ps = 1/(1+exp(-c(w %*% ps.model$coefficients))) 
...
IPTW.sym.RMST = function(L,w,x,z,time,delta,q,ps.model,ps.formula,censor.formula){
  # w is covariates vector for PS model, including the intercept term
  # x is covariates vector for censoring model
...
IPTW.asym.RMST = function(L,w,x,z,time,delta,q,ps.model,ps.formula,censor.formula){
  # w is covariates vector for PS model, including the intercept term
  # x is covariates vector for censoring model
  # original propensity score
  ps = 1/(1+exp(-c(w %*% ps.model$coefficients)))
...
RMST.Effect.BW = function(Data,L,Treatment,SurvTime,Status,ps.formula,
  ...
  cen.con.model = coxph(censor.formula, data=data.con)
  if (Method=="IPTW"){
    res = IPTW.RMST(L=L,w,x,z,time,delta,ps.model,cen.trt.model,cen.con.model)
  }else if (Method=="OW"){
    res = OW.RMST(L=L,w,x,z,time,delta,ps.model,cen.trt.model,cen.con.model)
  }else if (Method=="Symmetric"){
    res = IPTW.sym.RMST(L=L,w,x,z,time,delta,q=alpha,ps.model,ps.formula,censor.formula)
  }else if (Method=="Asymmetric"){
    res = IPTW.asym.RMST(L=L,w,x,z,time,delta,q=q,ps.model,ps.formula,censor.formula)
  }
  est = res$point
